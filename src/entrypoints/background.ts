...
export default defineBackground(() => {
  ...
  const findPortStateForBatch = (batchId: string): FillPortState | null => {
    for (const s of fillPorts) {
      if (s.batchId === batchId) return s
    }
    return null
  }
  ...
  const unregisterDynamicScripts = async (pattern: string): Promise<void> => {
    ...
    try {
      ...
      console.warn('[TP] unregisterDynamicScripts ignored', (e as Error).message)
    }
  }
  ...
  const reconcileRegisteredScripts = async (): Promise<void> => {
    ...
    try {
      ...
      console.warn('[TP] reconcileRegisteredScripts failed', (e as Error).message)
    }
  }

  const injectIntoActiveFrames = async (pattern: string): Promise<void> => {
    try {
      const originBase = originFromPattern(pattern)
      ...
      for (const tab of tabs) {
        ...
        for (const frame of frames) {
          const url = frame.url ?? ''
          if (!url.startsWith(originBase)) continue
          try {
            await browser.scripting.executeScript({
  ...
  const purgeInjectionMarkerForFrame = (
    ...
    mainWorldInjected.delete(`${tabId}:${frameId}`)
  }
  ...
  const buildBroadcastTabFilter = async (): Promise<(url: string) => boolean> => {
    const enabled = await getEnabledPatterns()
    ...
    return (url: string) => cachedBroadcastFilter!.regexes.some((r) => r.test(url))
  }
  ...
  const computeResolvedModeForFrame = async (
    ...
    const mode = await getOriginMode(pattern)
    if (mode === 'application') return 'application'
    if (mode === 'notesOnly') return 'notesOnly'
    try {
      const url = new URL(senderUrl)
      if (detectAtsHost(url.hostname) !== 'generic') return 'application'
    } catch {}
    return 'notesOnly'
  }
  ...
  const finalizeBatch = (batch: PendingFillBatch): void => {
    ...
    let total = 0
    for (const v of batch.totalByFrame.values()) total += v
    const done = batch.counts.filled + batch.counts.skipped + batch.counts.failed
    if (done < total) {
  ...
  const finalizeBatchForTab = (tabId: number, reason: string): void => {
    const batch = pendingFillBatches.get(tabId)
    if (!batch) return
  ...
  const recordFrameDeliveryFailure = (
    ...
    const portState = findPortStateForBatch(batchId)
    if (portState) {
      safePortPost(portState.port, {
        kind: 'fill.progress',
        counts: { ...batch.counts },
  ...
  const startFillForPort = async (state: FillPortState): Promise<void> => {
    ...
    if (pendingFillBatches.has(state.tabId)) {
      safePortPost(state.port, {
        kind: 'fill.error',
        error: 'A fill is already in progress for this tab',
      })
      return
    }
    const frames = await readFramesForTab(state.tabId)
    if (frames.length === 0) {
      const empty: FillBatchResult = {
        counts: { filled: 0, skipped: 0, failed: 0 },
        framesTimedOut: 0,
        total: 0,
      }
      safePortPost(state.port, { kind: 'fill.done', result: empty })
      return
    }
    ...
    port.onDisconnect.addListener(() => {
      ...
      if (state.batchId !== null && state.tabId >= 0) {
        ...
        if (batch && batch.batchId === state.batchId && !batch.finalized) {
          finalizeBatchForTab(state.tabId, 'port disconnected')
        }
      }
    })
  })
  ...
  const handleMessage = async (
    ...
    switch (message.kind) {
      ...
      case 'mode.get': {
        ...
        const mode = await computeResolvedModeForFrame(url)
      ...
      case 'resolve': {
        ...
        try {
          const profile = await getProfile(false)
          const value: ProfileValue = resolveField(
            message.fieldName,
            message.fieldType,
            message.section,
            profile,
      ...
      case 'origin.enable': {
        const result = await grantAndEnableOrigin(message.pattern, message.mode)
  ...
  browser.tabs.onRemoved.addListener(async (tabId) => {
    ...
    if (pendingFillBatches.has(tabId)) {
      finalizeBatchForTab(tabId, 'tab closed')
    }
    const attempt = await getConnectAttempt()
  ...
  if (browser.webNavigation?.onCommitted) {
    browser.webNavigation.onCommitted.addListener((details) => {
      if (details.frameId === 0) {
        void removeTabFromRegistry(details.tabId)
        purgeInjectionMarkersForTab(details.tabId)
        if (pendingFillBatches.has(details.tabId)) {
          finalizeBatchForTab(details.tabId, 'navigation')
        }
      } else {
        void removeFrameFromRegistry(details.tabId, details.frameId)
        purgeInjectionMarkerForFrame(details.tabId, details.frameId)
      }
    })
  } else {
    console.warn(
  ...
  browser.commands?.onCommand.addListener(async (command) => {
    ...
    if (command === 'open_picker') {
      ...
      browser.tabs
        .sendMessage(tab.id, { kind: 'cmd.fillAll' })
        .catch(() => {})
    }
  })
