...
export const parseLocation = (input: string | null | undefined): ParsedLocation | null => {
  ...
  if (parts.length === 0) {
    return { raw, country: null, city: null, region: null, remainder: raw }
  }

  let country: CountryInfo | null = null
  let countryIdx = -1
  for (let i = parts.length - 1; i >= 0; i--) {
    const info = lookupCountry(parts[i])
    if (info) {
      country = info
      countryIdx = i
      break
